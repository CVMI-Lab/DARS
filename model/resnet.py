...
class ResNet_inpaint(nn.Module):
    # nInputChannels = 4
    def __init__(self, block, layers, num_classes=1000, deep_base=True):
        super(ResNet_inpaint, self).__init__()
        self.deep_base = deep_base
        if not self.deep_base:
            self.inplanes = 64
            self.conv1 = nn.Conv2d(23, 64, kernel_size=7, stride=2, padding=3, bias=False)   # change
            self.bn1 = BatchNorm(64)
        else:
            self.inplanes = 128
            self.conv1 = conv3x3(23, 64, stride=2)   # change
            self.bn1 = BatchNorm(64)
            self.conv2 = conv3x3(64, 64)
            self.bn2 = BatchNorm(64)
            self.conv3 = conv3x3(64, 128)
...
def resnet50_inpaint(pretrained=False, **kwargs):
    """Constructs a ResNet-50 model.

    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet_inpaint(Bottleneck, [3, 4, 6, 3], **kwargs)
    # if pretrained:
    #     model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    #     model_path = './initmodel/resnet50_v2.pth'
    #     model_path = '/root/Workspace_he/from-git/HKU/PspNet/SSL-PSP/initmodel/resnet50_v2.pth'
