...
def test(test_loader, data_list, model, classes, mean, std, base_size, crop_h, crop_w, scales, gray_folder, color_folder, colors, names, args):
    print('>>>>>>>>>>>>>>>> Validate at full size >>>>>>>>>>>>>>>>')
    data_time = AverageMeter()
    batch_time = AverageMeter()
    model.eval()
    check_makedirs(gray_folder)
    print('Saving validate prediction to %s' % gray_folder)
    end = time.time()
    ...
    return cal_acc(data_list, gray_folder, classes, names, args)


def cal_acc(data_list, pred_folder, classes, names, args):
