...
class Compose(object):
    ...
    def __call__(self, image, label=None, base=False):
...
class ToTensor(object):
    ...
    def __call__(self, image, label=None):
        if label is None:
            ...
            if len(image.shape) == 2:
                image = np.expand_dims(image, axis=2)
            # if len(label.shape) != 2 or 3 or 4:
            #     print('in transform.py {}'.format(label.shape))
            #     raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 2 and 3 dims.\n"+str(len(label.shape))))
        ...
        if label is not None:
            ...
            if len(label.shape) == 3:
                ...
                for i in range(label.shape[0]):
                    label_trans = cv2.resize(label[i], None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
                    label_trans = np.expand_dims(label_trans, axis=0)
                    if soft_label is None:
                        soft_label = label_trans
                    else:
                        soft_label = np.concatenate((soft_label, label_trans), axis=0)
                label = soft_label
        return image, label
    ...
    def __call__(self, image, label=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if label is not None:
                ...
                if len(label.shape) == 3:
                    soft_label = None
                    for i in range(label.shape[0]):
                        label_trans = cv2.copyMakeBorder(label[i], pad_h_half, pad_h - pad_h_half, pad_w_half, pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
                        label_trans = np.expand_dims(label_trans, axis=0)
                        if soft_label is None:
                            soft_label = label_trans
                        else:
        ...
        if label is not None:
            ...
            if len(label.shape) == 3:
                soft_label = None
                for i in range(label.shape[0]):
                    label_trans = label[i,h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
                    label_trans = np.expand_dims(label_trans, axis=0)
                    if soft_label is None:
                        soft_label = label_trans
                    else:
        ...
        if random.random() < self.p:
            ...
            if label is not None:
                ...
                if len(label.shape) == 3:
                    soft_label = None
                    for i in range(label.shape[0]):
                        label_trans = cv2.warpAffine(label[i], matrix, (w, h), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_CONSTANT, borderValue=self.ignore_label)
                        label_trans = np.expand_dims(label_trans, axis=0)
                        if soft_label is None:
                            soft_label = label_trans
                        else:
